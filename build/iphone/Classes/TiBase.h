...
// in simulator we redefine to format for Depot Developer console
...
{\
	NSString * exceptionName = [@"org.depot." stringByAppendingString:NSStringFromClass([self class])];\
...
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.depot";\
	NSString * message = [NSString stringWithFormat:@"%@. %@ %@",reason,(subreason!=nil?subreason:@""),(location!=nil?location:@"")];\
	TiExceptionThrowWithNameAndReason(exceptionName,message);\
}\
...
enum {
	UIDepotNativeItemNone = -1, 
	UIDepotNativeItemSpinner = -2,
	UIDepotNativeItemProgressBar = -3,
	
	UIDepotNativeItemSlider = -4,
	UIDepotNativeItemSwitch = -5,
	UIDepotNativeItemMultiButton = -6,
	UIDepotNativeItemSegmented = -7,
	
	UIDepotNativeItemTextView = -8,
	UIDepotNativeItemTextField = -9,
	UIDepotNativeItemSearchBar = -10,
	
	UIDepotNativeItemPicker = -11,
	UIDepotNativeItemDatePicker = -12,
	
	UIDepotNativeItemInfoLight = -13,
	UIDepotNativeItemInfoDark = -14,
	
	UIDepotNativeItemDisclosure = -15,
	
	UIDepotNativeItemContactAdd = -16
};
...
/**
 *	TiThreadPerformOnMainThread should replace all Depot instances of
